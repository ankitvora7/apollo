...
Velodyne32Parser::Velodyne32Parser(Config config) : VelodyneParser(config) {
  for (int i = 0; i < 4; i++) {
    gps_base_usec_[i] = 0;
  ...
  if (config_.model == "32E") {
    mode_ = STRONGEST;
  } else if (config_.model == "32E_STRONGEST") {
    mode_ = STRONGEST;
  } else if (config_.model == "32E_LAST") {
    mode_ = LAST;
  }
}

void Velodyne32Parser::setup() {
...
void Velodyne32Parser::set_base_time_from_packets(
  ...
  static int year = 1, month = 1, day = 1, hour = 1, minute = 1,
             second = 1; // Hack to make this work. Initial value was -1 changed it to 1
  ...
  ROS_INFO("Get base time from packets. Obtained (%d.%d.%d %d:%d:%d) and status_type = %d and time = %f", year, month, day, hour, minute, second, GPS_STATUS,ros::Time::now().toSec());
...
void Velodyne32Parser::check_gps_status(
    const velodyne_msgs::VelodynePacket& pkt) {
  const RawPacket* raw = (const RawPacket*)&pkt.data[0];
  StatusType status_type = StatusType(raw->status_type);
  char status_value = raw->status_value;
...
void Velodyne32Parser::init_offsets() {
  int width = 32;
...
double Velodyne32Parser::get_timestamp(double base_time, float time_offset,
  ...
  index = block_id & 1;  // % 2 or block_id & 3
...
int Velodyne32Parser::intensity_compensate(const LaserCorrection& corrections,
...
void Velodyne32Parser::unpack(const velodyne_msgs::VelodynePacket& pkt,
  ...
  for (int i = 0; i < BLOCKS_PER_PACKET; ++i) {  // 12
    ...
    // ROS_INFO("The laser block id is : %d",raw->blocks[i].laser_block_id);
    int bank_origin = (raw->blocks[i].laser_block_id == UPPER_BANK) ? 0 : 16; // Wireshark say the paclet has ffee and not eeff
